...
printRegisters = do
  ...
  liftIO $ do
    ...
    let rs = [("A" :: String, a_), ("B", b_), ("C", c_), ("D", d_), ("E", e_),
              ("H", h_), ("L", l_)]
    mapM_ (\(n, fn) -> printf "%2s: 0x%02x\n" n . fn $ cpu) rs
    putStrLn ""

    putStrLn "Flags:"
    let fs = [("Z" :: String, 7), ("N", 6), ("H", 5), ("C", 4)]
    forM_ fs $ \(n, bn) ->
      printf "%2s: %d\n" n . bool 0 (1 :: Word8) . testBit (f_ cpu) $ bn
    putStrLn ""

    putStrLn "Special Registers:"
    let ss = [("PC" :: String, pc_), ("SP", sp_)]
    mapM_ (\(n, fn) -> printf "%3s: 0x%04x\n" n . fn $ cpu) ss
    printf "IME: %d\n" . either (const 0) (bool 0 (1 :: Word8)) . ime_ $ cpu
